...
{
    /// <summary>
    /// 获取默认的 <see cref="PurColumn"/> 实例，用于内部默认配置。
    /// </summary>
    ...
    public string IndexLetter => ExcelHelper.ToColumnLetter(_index);
    ...
    /// 如果列名集合为空，则返回 <see langword="null"/>。
    /// </summary>
    ...
    /// <summary>
    /// 获取或设置列宽是否为用户自定义设置的标志，用于区分自动计算的列宽和手动设置的列宽。
    /// </summary>
    ...
    {
        Index = ExcelHelper.ToColumnIndex(indexLetter);
        return this;
    }
